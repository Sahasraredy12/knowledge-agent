...
2. Start Ollama (it usually runs as a background service).  
...
Place your company / sample documents into the `docs/` folder, for example:
...
This step parses documents, chunks them, creates embeddings, and stores them in Chroma.

python ingest.py

text

If you change or add documents later, re‑run this script.  
You can safely delete the `vectorstore/` folder to rebuild from scratch.
...
1. Choose a **Workspace** from the sidebar:
   - All / HR Docs / Projects.
2. Type a question (e.g., “What is the notice period?” or “What does the yoga project do?”).
3. Click **Get Answer**.
4. View:
   - The concise answer.
   - **Sources (top matches)** for transparency.
   - **Chat history** to see the conversation flow.
5. Use **🧹 Clear chat history** in the sidebar to reset the conversation.
